ABOUT_WEATHER_REPORT_TEXT = f'\n\n*Управлять временем* можно командой /time (она же команда /шли).' \
    f' Ладно, управлять временем нельзя. Эх. Но мы попробуем' \
    f'\n\n/time (она же /шли) -- показать это описание и список уже настроенных сводок' \
    f'\n\n/time 13 (или /шли 13) -- отправлять сводку погоды каждый день в 13 часов' \
    f'\n\n/time 13.30 (или /шли 13.30) -- отправлять сводку погоды каждый день в 13 часов 30 минут' \
    f'\n\n/time сб 13.30 (или /шли сб 13.30) -- отправлять сводку погоды каждую субботу в 13 часов 30 минут' \
    f'\n\n/time clear (или /шли никогда) -- очистить список сводок (т.е. не отправлять сводки погоды)' \
    ...
    f'\n\n/home караганда (она же /дом караганда) -- установить город в качестве домашнего.' \
...
START_TEXT = f'Здравствуйте! Бот показывает погоду в выбранном месте' \
    ...
    f' Например: вместо\n\n/Караганда\n\nможно написать\n\nкараганда' \
    f'\n\n\n\n*Основые команды*' \
    f'\n\n/караганда (вместо караганда любой город) -- показывает погоду в городе.' \
    f' Возможно, придётся выбрать один из нескольких предложенных городов' \
    f'\n\n/here (она же команда /тут) -- показывает погоду в вашей' \
    f' геопозиции (работает, только если писать боту в личку).' \
    f'\n\n/dark (она же /тьма) -- поменять цвет картинки со светлого на тёмный, либо обратно' \
    f'\n\n\n\n*Сводки погоды*' \
    ...
    f'\n\n*Списком городов* можно управлять командами /add (/добавь), /list (/список),' \
    f' /report (/сводка) и /clear (/очисти).' \
    f'\n\n/add (она же /добавь) -- добавить город в список. Например, /добавь караганда' \
    f'\n\n/list (она же /список) -- показать список уже добавленных городов' \
    f'\n\n/report (она же /сводка) -- показать погоду в городах из списка' \
    f'\n\n/clear (она же /очисти) -- удалить все города из списка' \
